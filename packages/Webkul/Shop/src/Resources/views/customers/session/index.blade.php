        ...
        <form method="POST" action="{{ route('customer.session.create') }}" @submit.prevent="onSubmit">
            ...
            <div class="login-form">
                ...
                <div class="control-group" :class="[errors.has('username') ? 'has-error' : '']">
                    <label for="username" class="required">{{ __('shop::app.customer.login-form.username') }}</label>
                    <input type="text" class="control" name="username" v-validate="'required'" value="{{ old('username') }}" data-vv-as="&quot;{{ __('shop::app.customer.login-form.username') }}&quot;">
