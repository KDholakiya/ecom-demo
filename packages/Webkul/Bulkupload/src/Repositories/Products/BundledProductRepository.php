        ...
        try  {
            ...
            if ($requestData['numberOfCSVRecord'] > $processCSVRecords) {
                for ($i = $requestData['countOfStartedProfiles']; $i < $uptoProcessCSVRecords; $i++)
                {
                    ...
                    if((array_key_exists("up_sell" , $csvData[$i]))){
                        $products = explode(',', $csvData[$i]['up_sell']);
                        foreach ($products as $key => $value)
                        {
                            $product = $this->productRepository->where(['sku' => $products[$key]])->value("id");
                            if($product != null){
                                $data['up_sell'][] = $product;
                            }
                        }
                    }
     
                    if((array_key_exists("cross_sell" , $csvData[$i]))){
                        $products = explode(',', $csvData[$i]['cross_sell']);
                        foreach ($products as $key => $value)
                        {
                            $product = $this->productRepository->where(['sku' => $products[$key]])->value("id");
                            if($product != null){
                                $data['cross_sell'][] = $product;
                            }
                        }
                    }
    
                    if((array_key_exists("related_products" , $csvData[$i]))){
                        $products = explode(',', $csvData[$i]['related_products']);
                        foreach ($products as $key => $value)
                        {
                            $product = $this->productRepository->where(['sku' => $products[$key]])->value("id");
                            if($product != null){
