...
class CategoryRepository extends Repository
{
    ...
    public function getLeafCat($category){
        $arrr = [];
        if($category['children']->count()){
            foreach ($category['children'] as $key => $subcat) {
                $retSub = $this->getLeafCat($subcat);
                $arrr = array_merge($arrr,$retSub);
            }
        }
        else{
            array_push($arrr,$category);
        }
        return $arrr;
    }

    public function getVisibleLeafCategory($id = null)
    {
        $categories = $this->getCategoryTreeWithoutDescendant();
        return $this->getLeafCat($categories[0]);
    }
