...
class Cart
{
    ...
    ): void
    {
        ...
        if ($billingAddressModel) {
            $temp = $billingAddressModel->toArray();
            if(array_key_exists("use_for_shipping",$temp)){unset($temp['use_for_shipping']);}
            if(array_key_exists("address_id",$temp)){unset($temp['address_id']);}
            if(array_key_exists("save_as_address",$temp)){unset($temp['save_as_address']);}
            $this->cartAddressRepository
            ...
            if ($cart->haveStockableItems()) {
                ...
                if ($shippingAddressModel) {
                    if (isset($billingAddressData['use_for_shipping']) && $billingAddressData['use_for_shipping']) {
                        $temp = $billingAddressData;
                        if(array_key_exists("use_for_shipping",$temp)){unset($temp['use_for_shipping']);}
                        if(array_key_exists("address_id",$temp)){unset($temp['address_id']);}
                        if(array_key_exists("save_as_address",$temp)){unset($temp['save_as_address']);}
                        $this->cartAddressRepository->where('address_type',"=",CartAddress::ADDRESS_TYPE_BILLING)
                        ->where('id',"=",$shippingAddressModel->id)
                        ->update($temp);;
                    } else {
                        $temp = $shippingAddressData;
                        if(array_key_exists("use_for_shipping",$temp)){unset($temp['use_for_shipping']);}
                        if(array_key_exists("address_id",$temp)){unset($temp['address_id']);}
                        if(array_key_exists("save_as_address",$temp)){unset($temp['save_as_address']);}
                        $this->cartAddressRepository->where('address_type',"=",CartAddress::ADDRESS_TYPE_BILLING)
